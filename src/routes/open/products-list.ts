...
export class ProductsListOpenAPIRoutes {


    public routes(server: Application) {


        server.route('/api/open/:token/products/list')
        ...
        server.route('/api/open/products/list')
            .get(async (req: Request, res: Response) => {
                ...
                try {
                    ...
                    return res.redirect(
                        url.format({
                            pathname: `/api/open/${token_value}/products/list`,
                            query: req.query
                        })
                    );
